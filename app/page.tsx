...
import { DEFAULT_COMMANDPALETTE_STATE } from "./_data/CommandPalettePresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("command-palette-component");
  ...
  const update = <K extends keyof CommandPaletteState>(key: K, value: CommandPaletteState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_COMMANDPALETTE_STATE, ...(preset.state as Partial<CommandPaletteState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
